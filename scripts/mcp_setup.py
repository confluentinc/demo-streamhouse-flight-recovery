...
_CLAUDE_SERVER_NAME = "confluent-cloud-mcp-server"


def _claude_argv_stdio(
    env_vars: dict, npx_bin: str
) -> tuple[list[str], list[str]]:
    """(remove, add) argv for the local stdio Confluent MCP server.

    Stdio invocation:
    `claude mcp remove <name> -s local` then `claude mcp add --scope local <name>
    --env K=V ... -- <cmd>`. Secrets ride as `--env` flags (same plaintext exposure
    as the ~/.claude.json the CLI ultimately writes).
    """
    add_argv = ["claude", "mcp", "add", "--scope", "local", _CLAUDE_SERVER_NAME]
    for key, value in env_vars.items():
        add_argv += ["--env", f"{key}={value}"]
    add_argv += ["--", npx_bin, "-y", "@confluentinc/mcp-confluent"]
    return (
        ["claude", "mcp", "remove", _CLAUDE_SERVER_NAME, "-s", "local"],
        add_argv,
    )
...
def _register_with_claude_code(
    ...
    """Register the local stdio Confluent MCP server with Claude Code.

    Primary path runs `claude mcp add --scope local`.
    Falls back to writing ~/.claude.json directly when the `claude` binary is absent
    or the CLI call fails (e.g. a managed-device policy blocks it). The direct
    write produces a byte-identical projects[...].mcpServers entry, so either path
    lands the same config.
    """
    remove_argv, add_argv = _claude_argv_stdio(env_vars, npx_bin)
    try:
        subprocess.run(remove_argv, capture_output=True)
        result = subprocess.run(add_argv, capture_output=True, text=True)
        if result.returncode == 0:
            print(
                f"✓ Confluent MCP server registered as '{_CLAUDE_SERVER_NAME}' "
                "(local scope, via claude CLI)"
            )
    ...
    _register_with_claude_code_filewrite(env_vars, npx_bin, project_root)


def _register_with_claude_code_filewrite(
    env_vars: dict, npx_bin: str, project_root: Path
) -> None:
    """Fallback: write the MCP server entry into ~/.claude.json under the project key.

    Produces the same entry `claude mcp add --scope local` would, without needing the
    `claude` binary to be present or allowlisted. (settings.json/settings.local.json
    don't support mcpServers, so this file is the right target.)
    """
    ...
    print(
        f"✓ Confluent MCP server registered as '{_CLAUDE_SERVER_NAME}' "
        "(local scope, direct write)"
    )
    print("  Restart Claude Code to activate.")


def _write_codex_config(config_path: Path, env_vars: dict, npx_bin: str) -> None:
    """Merge an `[mcp_servers.<name>]` stdio entry into one Codex config.toml, in place.

    Uses tomlkit (already a declared dependency, and the same writer setup_rtce.py uses)
    so a round-trip preserves the rest of the file's comments/formatting. This replaces
    an earlier tomli_w/tomllib import that was never declared in pyproject, so it failed
    at runtime with ModuleNotFoundError. If the existing file can't be parsed it's backed
    up to <name>.toml.bak and rewritten fresh — we never print the server's secret env
    ...
    entry = tomlkit.table()
    entry["command"] = npx_bin
    entry["args"] = ["-y", "@confluentinc/mcp-confluent"]
    env_table = tomlkit.table()
    for key, value in env_vars.items():
        env_table[key] = value
    entry["env"] = env_table
    mcp_servers["confluent-cloud-mcp-server"] = entry
...
def main():
    ...
    # Homebrew's npx launcher uses `#!/usr/bin/env node`. An absolute npx path
    # alone therefore does not guarantee that it runs with the sibling Node
    # version we selected above; a newer PATH-level Node can win and fail to load
    # kafka-javascript's prebuilt native module with an ABI mismatch. Pin the
    # selected Node directory first in the MCP server process environment.
    if npx_bin != "npx":
        node_bin_dir = str(Path(npx_bin).parent)
        inherited_path = os.environ.get("PATH", "")
        env_vars["PATH"] = (
            f"{node_bin_dir}{os.pathsep}{inherited_path}"
            if inherited_path
            else node_bin_dir
        )
